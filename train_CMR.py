...
from datasets.dataloader import Dataset_Mapping
from model.contrast_aug_2D import GINGroupConv
...
def parse_args():
    ...
    parser.add_argument('--varlr', type=float, default=1e-4, help='variance model learning rate')
    ...
    parser.add_argument('--datapath', type=str, default='/data/yinsong/T1Map_Training', help='training data path')
    parser.add_argument('--model_dir', type=str, default='../Model_CMR', help='model save directory')
    parser.add_argument('--log_dir', type=str, default='../Log_CMR', help='tensorboard log directory')
...
def train(args):
    ...
    for name in names:
        ...
        moving = sorted(glob.glob(name + '/s*_t0_warped.nii.gz'))
        pair = list(itertools.product(moving, fixed))
        index_pair += pair

    train_dataset = Dataset_Mapping(index_pair, norm=True)
