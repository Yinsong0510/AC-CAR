...
from datasets.dataloader import Dataset_epoch_validation
...
IMAGE_SIZE = (192, 160, 144)


def parse_args():
    parser = ArgumentParser()
    parser.add_argument('--datapath', type=str, default='../../IXI_final/IXI-test', help='test data path')
    parser.add_argument('--model_dir', type=str, default='../Model_IXI/', help='model directory')
    ...
    parser.add_argument('--contrast_1', type=str, default='/IXI-T2', help='contrast 1 folder')
    parser.add_argument('--contrast_2', type=str, default='/IXI-PD', help='contrast 2 folder')
...
def test(args):
    ...
    name_1 = sorted(glob.glob(args.datapath + args.contrast_1 + '/IXI*.nii.gz'))
    name_2 = sorted(glob.glob(args.datapath + args.contrast_2 + '/IXI*.nii.gz'))
    label = sorted(glob.glob(args.datapath + '/IXI-test-seg/IXI*/IXI*_MALPEM_tissues.nii.gz'))

    test_dataset = Dataset_epoch_validation(name_1, name_2, label, norm=True)
    test_loader = Data.DataLoader(test_dataset, batch_size=1, shuffle=True, num_workers=2)

    reg_model_path = os.path.join(args.model_dir, args.reg_model)
    var_model_path = os.path.join(args.model_dir, args.var_model)
    log_path = os.path.join(args.model_dir, 'Results_t2pd.txt')
    ...
    for data in test_loader:
        ...
        with torch.no_grad():
            ...
            aligned_label_hot = one_hot(aligned_label, 6)
            fixed_label_img_hot = one_hot(fixed_label_img, 6)

            hd95_score = monai.metrics.compute_hausdorff_distance(
                aligned_label_hot, fixed_label_img_hot, percentile=95)[0, 1]
